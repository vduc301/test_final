$(document).ready(function(){
	...
	function ajaxGet(page){
		$.ajax({
			type: "GET",		
			url: "http://localhost:8088/laptopshop/api/nhan-hieu/all?page=" + page,
				...
				$.each(result.content, function(i, nhanHieu){
					var nhanHieuRow = '<tr style="text-align: center;">' +
					                  ...
					                  '<td>'+'<input type="hidden" value=' + nhanHieu.id + '>'
					                     + '<button class="btn btn-primary btn-outline btnCapNhatNhanHieu" >Cập nhật</button>' +
					                     '   <button class="btn btn-danger btn-outline btnXoaNhanHieu">Xóa</button></td>'
				                      '</tr>';
	...
	function ajaxPost(){
    	...
    	$.ajax({
			...
			url : "http://localhost:8088/laptopshop/api/nhan-hieu/save",
    ...
    $(document).on("click",".btnCapNhatNhanHieu",function(){
		...
		var href = "http://localhost:8088/laptopshop/api/nhan-hieu/" + nhanHieuId;
    ...
    function ajaxPut(){
    	...
    	$.ajax({
			...
			type : "PUT",
			contentType : "application/json",
			url : "http://localhost:8088/laptopshop/api/nhan-hieu/update",
			data : JSON.stringify(formData),
            // dataType : 'json',
			success : function(response) {
    ...
    $(document).on("click",".btnXoaNhanHieu", function() {
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8088/laptopshop/api/nhan-hieu/delete/" + nhanHieuId,
