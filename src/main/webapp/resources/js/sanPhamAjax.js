$(document).ready(function() {
	...
	function ajaxGet(page){
		...
		$.ajax({
			...
			url: "http://localhost:8088/laptopshop/api/san-pham/all" + '?page=' + page,
			success: function(result){
				$.each(result.content, function(i, sanPham){
					var sanPhamRow = '<tr>' +
					                  ...
					                  '<td> <button class="btn btn-warning btn-outline btnChiTiet" style="margin-right: 6px">Chi tiết</button>' ;
					
					var checkTenDanhMuc = (sanPham.danhMuc.tenDanhMuc.toLowerCase()).indexOf("Laptop".toLowerCase());
					sanPhamRow += ( checkTenDanhMuc != -1)?'<button class="btn btn-primary btn-outline btnCapNhatLapTop" >Cập nhật</button>':'<button class="btn btn-primary btnCapNhatOther" >Cập nhật</button>';
					sanPhamRow += '  <button class="btn btn-danger btn-outline btnXoaSanPham">Xóa</button></td>'+'</tr>';
    ...
    function ajaxPostLapTop() {
    	// PREPATEE DATA
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8088/laptopshop/api/san-pham/save",
    ...
    $(document).on("click",".btnCapNhatLapTop", function(event){
		...
		var href = "http://localhost:8088/laptopshop/api/san-pham/"+sanPhamId;
    ...
    $(document).on("click",".btnXoaSanPham", function() {
		...
		if(confirmation){
		  $.ajax({
			  async:false,
			  type : "DELETE",
			  url : "http://localhost:8088/laptopshop/api/san-pham/delete/" + sanPhamId,
