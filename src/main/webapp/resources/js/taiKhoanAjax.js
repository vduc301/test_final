$(document).ready(function() {
	...
	function ajaxGet(page){		
		...
		$.ajax({
			...
			url: "http://localhost:8088/laptopshop/api/tai-khoan/all" + '?page=' + page,
...
//					                  '<td><button class="btn btn-primary btnCapNhat" >Cập nhật</button></td>' + 
					                  '<td><button class="btn btn-danger btn-outline btnXoa" >Xóa</button></td>';			;
    ...
    function ajaxPostTaiKhoan() {    	
    	...
    	console.log(data);
    	 ...
    	 $.ajax({
 			...
 			type : "POST",
 			contentType : "application/json",
 			url : "http://localhost:8088/laptopshop/api/tai-khoan/save",
    ...
    $(document).on("click",".btnXoa", function() {
		...
		if(confirmation){
		  $.ajax({
			  type : "DELETE",
			  url : "http://localhost:8088/laptopshop/api/tai-khoan/delete/" + taiKhoanId,
